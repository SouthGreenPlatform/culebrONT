...
class CulebrONT(object):
    ...
    def __check_config_dic(self, config):
        ...
        ##### CHECK KAT
        if bool(self.config["QUALITY"]["KAT"]) or bool(self.config["CORRECTION"]["PILON"]) or bool(self.config["QUALITY"]['FLAGSTATS']):
        ...
        # check singularity image for conda (contains a lot of conda envs)
        self.__check_tools_config("SINGULARITY", "CONDA", ["CONDA"])

        ## TODO : check module envs if --use-moduleenvs is activated on command line
        ##
        ...
        self.nb_pilon_rounds = '1' if not bool(self.config['CORRECTION']['PILON']) else str(config['params']['PILON']['PILON_ROUNDS'])
