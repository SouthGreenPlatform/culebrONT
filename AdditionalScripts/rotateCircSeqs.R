...
if (circLogProvided && any(circularInName)) { # if log and flagged seq, sanity check on names
  ...
  seqNamePrefixes <- sapply(names(seq), function(x) {strsplit(x, split = "[| ]")[[1]][1]})
  circNamePrefixes <- sapply(circlatorDf$X.Contig, function(x) {strsplit(x, split = "[| ]")[[1]][1]})
...
if (length(circSeq) != 0) {
  names(newCircSeq) <- if (any(circularInName)) {
    paste(names(newCircSeq), "rotated", sep = "|")
  } else if (circLogProvided && !any(circularInName)) {
    paste(names(newCircSeq), "circular|rotated", sep = "|")
  }
}
