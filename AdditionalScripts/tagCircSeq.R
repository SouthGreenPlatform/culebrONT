...
if(grepl("cov.\\tcirc.", firstLine)) {
  ...
  newseq <- seq
  logInfo$title <- ifelse(logInfo$circ. %in% c("Y", "+"), paste(logInfo[[1]], circularTag, sep = "|"), logInfo[[1]])
  ...
  logInfo <- read.delim(logFile, stringsAsFactors = FALSE)
  newseq <- seq
  logInfo$title <- ifelse(logInfo$circularised == 1, paste(logInfo$X.Contig, circularTag, sep = "|"), logInfo$X.Contig)
  ...
  if (length(circDiagLines) != 0L) {
    circNames <-unique(sub("^L\\t(.*?)\\t.*0M$", "\\1", circDiagLines))
    names(newseq) <- ifelse(names(newseq) %in% circNames,
                            paste(names(newseq), circularTag, sep = "|"),
                            names(newseq)
                            )
  }
