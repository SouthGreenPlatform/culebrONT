...
weesam_dirs <- r_to_py(as.list(snakemake@input$weesam_dirs))
...
dico_tools = {
    ...
    "WEESAM" : "* WEESAM : https://github.com/centre-for-virus-research/weeSAM"
}
tools = []
...
for (i in 1:length(samples_list)) {
  ...
  busco = c(busco, knit_expand(text="## {{samples_list[[i]]}}\n\n{{dt}}\n\n{{plotly::ggplotly(p)}}\n\n", 
                                dt = DT::datatable(py_to_r(py$dico_busco[[samples_list[[i]]]]),
                                                  ...
                                                  option= list(paging=FALSE,searching = TRUE,ordering=TRUE, scrollX = FALSE, dom = 'Blfrtip'))
                                )
            )
}
```
...
#quast_files
```{python, results='asis', echo=FALSE, message = FALSE, warning = FALSE, eval=('FRC' %in% quality_tools_vector)}
print("""# FRC{.tabset .tabset-fade}

FRCbam tool computes Feature Response Curves in order to validate and rank assemblies and assemblers.
""")
sample_found = []
for frc_file in r.frc_files:
    sample = Path(f"{frc_file}").parent.name
    name = Path(f"{frc_file}").name
    print(f"## {sample}\n\n")
    path = Path(f"{frc_file}").parent
    curve = path.joinpath("frc.png").as_posix()
...
```{python, results='asis', echo=FALSE, message = FALSE, warning = FALSE, eval=('WEESAM' %in% quality_tools_vector)}
print("""# WEESAM {.tabset .tabset-fade}\n\n""")
sample_found = []
for weesam_dir in r.weesam_dirs:
    sample = Path(f"{weesam_dir}").parent.name.split("__")[0]
    assembler = Path(f"{weesam_dir}").parent.name.split("__")[1]
    quality_step = Path(f"{weesam_dir}").parent.name.split("__")[2]
    path = Path(f"{weesam_dir}")
    
    #html = path.joinpath("minimap2mapping_html_results/minimap2mapping.html").as_posix()
    html = f"./WEESAM/{sample}/{sample}__{assembler}__{quality_step}/minimap2mapping_html_results/minimap2mapping.html"
    ...
    print(f'<a href="{html}" target="_blank">Open WEESAM report on new window</a>\n\n')
    print('<div class="embed-responsive embed-responsive-4by3">\n')
    print(f'<iframe class="embed-responsive-item" src="{html}" loading="lazy" allowfullscreen></iframe>\n')
    print('</div>\n')
```
